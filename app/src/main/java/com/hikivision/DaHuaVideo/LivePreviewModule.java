...
public class LivePreviewModule {
    ...
    public LivePreviewModule(IPLoginModule mmLoginModule){
        mPlayPort = IPlaySDK.PLAYGetFreePort();
        isOpenSound = true;
        isDelayPlay = false;
        mLoginModule = mmLoginModule;
        initMap();
    }
    ///码流类型的hash
    ...
    public void startPlay(int channel,int streamType,final SurfaceView view){
        if(mRealHandle!=0) stopRealPlay();
        Log.d(TAG,"StreamTpye: "+streamTypeMap.get(streamType));
        mRealHandle = INetSDK.RealPlayEx(mLoginModule.getLoginHandle(),channel,streamTypeMap.get(streamType));
        if (mRealHandle == 0){
            Log.e(TAG,"RealPlayEx failed!");
...
//                        Log.i(TAG,"dataType == 0");
    ...
    public int getChannel(){
        if (mLoginModule == null)
            return 0;
        return mLoginModule.getDeviceInfo().nChanNum;
    }
    ///获取要显示的通道号
